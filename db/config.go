...
func (c Config) ConnectionString() (string, error) {
	...
	case "mysql":
		return fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?timeout=%ds&readTimeout=%ds&writeTimeout=%ds", c.User, c.Password, c.Host, c.Port, c.DatabaseName, c.Timeout, c.Timeout, c.Timeout), nil
	default:
